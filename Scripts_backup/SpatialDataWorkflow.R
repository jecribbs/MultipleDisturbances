# Spatial Data Workflow 2: add plot end points
# 08 January 2025
# Author: Jenny Cribbs
# Inputs: list of utms entered for each plot and/or kml file from GPS
# Code Description: (1) spatial data wrangling to use plot beginning waypoints in utms to calculate plot end waypoints in utms based on azimuth and transect length in meters (2) visualize points (3) choose calculated or gps waypoint to get one end point per plot (4) visualize result (5) convert all points to decimal degrees.
# Output: a csv file with lat/long coordinates for each plot beginning and end point.
...
## Part 1: Read in Yosemite field data
...
# Part 2: plot end point calculation and review

# Convert azimuth from degrees to radians
azimuth_rad <- azimuth * pi / 180

# Calculate the easting and northing offsets
delta_easting <- transect_length * sin(azimuth_rad)
delta_northing <- transect_length * cos(azimuth_rad)

# Calculate the ending UTM coordinates
end_easting <- start_easting + delta_easting
end_northing <- start_northing + delta_northing

# Create a data frame with the UTM coordinates
utm_coords <- data.frame(easting = end_easting, northing = end_northing)
utm_sf <- st_as_sf(utm_coords, coords = c("easting", "northing"), crs = 32611)  # Adjust UTM Zone CRS

# Transform to WGS84 (EPSG:4326, decimal degrees)
decimal_degrees_sf <- st_transform(utm_sf, crs = 4326)


# Part 2: Visualize the Points
...
st_crs(YPEplots)  # Should output WGS84 (EPSG:4326)
head(st_coordinates(YPEplots))  # Show the latitude and longitude
st_crs(kml) # also WGS84
head(st_coordinates(kml))
st_crs(nps) # 6269
...
# Part 5: Convert from utms to decimal degrees
...
tm_shape(nps) +
  ...
  tm_layout(main.title = "Plots in Yosemite", 
            ...
            frame = FALSE)
# Seems to be working
# Now need to filter for plot beginning waypoints only --> plot points
# Need to calculate missing plot ends then could plot ends too
