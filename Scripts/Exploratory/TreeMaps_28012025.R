...
calculate_tree_positions2 <- function(data) {
  data %>%
    mutate(
...
tree_positions2 <- calculate_tree_positions2(treeData)
...
# Map non-pila trees with plot points and gps points
tmap_mode("view")
tm_shape(nps) +
  tm_polygons(col = "gray",
              title = "Yosemite") +
...
pilaData <- left_join(pilaData, plotData)
...
# zone NA
unique(check$UTM_zone)
# find out where the NAs are
check %>% 
  filter(is.na(UTM_zone)) %>% 
  select(eventID, UTM_zone, PILA_UTM_E, PILA_UTM_N)
# fill in zone 11 for plots missing zone
pilaData_trimmed <- check %>%
  mutate(UTM_zone = ifelse(is.na(UTM_zone), 11, UTM_zone))
