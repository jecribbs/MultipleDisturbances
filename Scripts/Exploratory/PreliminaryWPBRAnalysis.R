...
## unzipping files from YPE_Data--download from Google Drive
...
perfig <- summarydat %>% 
  ...
  ylab("% infected WPBR") + xlab("Elevation (ft)") + 
...
perfigb <- summarydatbeetles %>% 
  ...
  geom_point()+ geom_smooth(method="lm")+
  ylab("% beetle infested") + xlab("Elevation (ft)") + 
  scale_x_discrete(guide = guide_axis(angle = 90)) +
  scale_color_viridis(discrete = TRUE, option = "D")+
  scale_fill_viridis(discrete = TRUE) +
  guides(fill=F, color=F)
...
#19 hs plots
hs <- filter(piladat, plot_type == "highseverity" | plot_type == "high severity")
# 36 random plots
